 ...
 * 
 *	v1.00 -- rewrite in C#.
 *	v1.01 -- door can now stay open.
 *	v1.02 -- made code more robust.
 *	v1.03 -- bug fix (if door was rotated, pivot was wrong).
 *	v1.04 -- added keyboard control.
 *	v1.11 -- added close-by-keypress.
 *	v1.13 -- bug-fix: door would not close properly
 *	v1.17 -- added handle turn
 *
 */
...
public class DoorSwing : MonoBehaviour {
	...
	void Update() {
		...
		if (open && !wasOpen && openingSound) {
			//audiosource.volume = volume;
			audiosource.PlayOneShot(openingSound);
			handle.Start();
		} else if (!open && wasOpen && closingSound) {
			//audiosource.volume = volume;
		...
		if (door) door.transform.RotateAround(pivotAbsolute, swingAxis, (travel - prevTravel) * swingAngle);
		prevTravel = travel;
		handle.Update();
